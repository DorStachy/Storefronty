...
export const config = {
  ...
  // real money so the full pick-plan→paid→permanent flow runs end-to-end; flipping to live Paddle is
  // purely adding keys. SAFETY: real keys ALWAYS win — the stub can never fire when Paddle/Stripe keys
  // are present (so a stray PAYMENTS_PROVIDER=stub in prod can't make every plan free).
  payments: { provider: (E.PADDLE_CLIENT_TOKEN ? 'paddle' : (E.STRIPE_SECRET_KEY ? 'stripe' : ((E.PAYMENTS_STUB === '1' || E.PAYMENTS_PROVIDER === 'stub') ? 'stub' : (E.PAYMENTS_PROVIDER || 'none')))) },
  paddle: {
